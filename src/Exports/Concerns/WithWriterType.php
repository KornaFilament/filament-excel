...
{
    protected Closure|string|null $exportWriterType = null;

    public function withWriterType(Closure|string|null $writerType = null): static
    {
        $this->exportWriterType = $writerType;
    ...
    {
        return $this->evaluate($this->exportWriterType) ?? Excel::XLSX;
    }

    protected function resolveWriterType(): void
    {
        if ($writerType = data_get($this->formData, 'writer_type')) {
            if ($this->exportWriterType instanceof Closure) {
                $writerType = $this->evaluate($this->exportWriterType, ['writerType' => $writerType]);
            }
